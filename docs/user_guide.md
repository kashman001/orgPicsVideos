...
6. Review the planned destination structure tree; new folders are blue, existing folders are black, and skipped files are gray. Labels also show status (existing/new/skipped), and folder/file icons help distinguish types. Expand a leaf folder to see the files that will be copied there.
7. Review the execution status tree; items that already exist show as black (existing). Items pending execution start as purple, then turn green on success or red on failure as the copy runs.
